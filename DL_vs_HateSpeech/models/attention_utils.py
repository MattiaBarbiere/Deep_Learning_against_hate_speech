...
def plot_attention_rollout(path, device="cpu"):
    ...
    # Perform self-attention rollout
    self_attention_rollout(model, text, image)
...
def self_attention_rollout(model, text, image):
    """
    Perform self-attention rollout for a given model and inputs.

    Args:
        model: The model to perform self-attention rollout on. Must have a method called get_model_attention.
        image: The image input for the model.
        text: The text input for the model.
    Returns:
        Tensor: The attention weights from the model of dims (text_shape, image_shape).
    ...
    # Extract the attention weights for the image
    attn_cumulative_tuple = [classifier_attn[0][:, :dim_image_mat, :dim_image_mat]]
    print("Shape of clip image attention:", attn_cumulative_tuple[0].shape)
    for i in range(1, len(classifier_attn)):
        attn_cumulative_tuple.append(classifier_attn[i][:, :dim_image_mat, :dim_image_mat])
        print("Shape of classifier attention:", classifier_attn[i].shape)
